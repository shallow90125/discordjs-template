...
for (const k in gsr) {
  ...
  if (s !== n) {
    throw new Error(`Command name (${n}) different from file name (${s}) (${k})`)
  }

  if (sr[`${c}.${g}`]) {
    throw new Error(`DiscordSubcommand (${c}.${g}) must export DiscordSubcommandGroup`)
  }
...
for (const k in gr) {
  ...
  if (g !== n) {
    throw new Error(`Command name (${n}) different from file name (${g}) (${k})`)
  }
...
export const commands: RESTPostAPIChatInputApplicationCommandsJSONBody[] = Object.values(cr).map((v) => v.option)
